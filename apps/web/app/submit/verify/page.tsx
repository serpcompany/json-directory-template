...
import { BadgePreview } from '@/components/verify/BadgePreview';
import { CopySnippet } from '@/components/verify/CopySnippet';
  ...
  useEffect(() => {
    ...
    fetch(`/api/submission?token=${encodeURIComponent(token)}`)
      .then(async (res) => {
        ...
        const data = (await res.json()) as SubmissionInfo;
      ...
      <div className="mx-auto max-w-2xl space-y-8">
        ...
        <details className="text-sm text-muted-foreground">
          ...
          <ul className="mt-2 list-disc space-y-1 pl-5">
            ...
            <li>
              Make sure the <code className="rounded bg-muted px-1 font-mono">data-verify-token</code>{' '}
              attribute is present and unchanged.
            </li>
            <li>Your site must be publicly accessible (not behind a login or Cloudflare challenge).</li>
            <li>DNS changes can take up to 48 hours to propagate — wait before retrying.</li>
          </ul>
        </details>
      </div>
    </div>
