jest.mock('@/lib/content-loader', () => ({
  getDocs: jest.fn(() => [{ slug: 'commands' }, { slug: 'getting-started' }]),
  getGuides: jest.fn(() => [{ slug: 'how-to-download-onlyfans-profiles-videos-images' }]),
  getWebsites: jest.fn(() => [
    {
...
describe('sitemap routes', () => {
  ...
  it('redirects sitemap.xml to the canonical sitemap index', async () => {
    ...
    expect(response.headers.get('location')).toBe('https://example.com/sitemap-index.xml')
  })
