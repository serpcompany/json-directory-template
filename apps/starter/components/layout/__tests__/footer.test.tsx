...
describe('Footer', () => {
  it('renders the current site config values', () => {
    ...
    if (siteConfig.features.showBrands) {
      ...
      expect(screen.queryByRole('link', { name: siteCopy.brandsLabel })).not.toBeInTheDocument()
    }
    ...
    expect(screen.getByRole('link', { name: 'DMCA' })).toHaveAttribute('href', getRoute('dmca'))
