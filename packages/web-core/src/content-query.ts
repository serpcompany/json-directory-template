...
export function resolveGuideBySlug(
  guides: GuideEntry[],
  slug: string
): GuideMetadata | null {
  const guide = guides.find(
    (currentGuide) => currentGuide.slug === slug && currentGuide.published
  );
...
export function resolveDocBySlug(
  docs: DocEntry[],
  slug: string
): DocMetadata | null {
  const doc = docs.find(
    (currentDoc) => currentDoc.slug === slug && currentDoc.published
  );
...
export function resolveAboutPage(
  aboutPages: AboutPageEntry[]
): AboutPageMetadata | null {
  const aboutPage = aboutPages.find(
    (page) => page.slug === 'about' && page.published
  );
