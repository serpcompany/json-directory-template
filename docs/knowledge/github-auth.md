...
- The starter now uses GitHub OAuth through `next-auth` in `apps/starter`.
...
- `apps/starter/lib/auth.ts` intentionally returns a safe signed-out state when those env vars are missing so the site still builds and the login page can explain what is missing.
