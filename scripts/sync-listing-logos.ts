...
function resolveManifestArg(argv: string[]): string {
  ...
  if (!manifestPath) {
    throw new Error('sync-listing-logos requires --manifest <path-to-logo-source-manifest>.')
  }
...
function parseManifest(path: string): LogoSourceManifestEntry[] {
  ...
  return parsed.map(entry => ({
    ...
    sourceUrl: typeof entry.sourceUrl === 'string' ? entry.sourceUrl : entry.sourceUrl,
    title: entry.title
  }))
}

function resolveLogoExtension(sourceUrl: string, contentTypeHeader: string | null): string {
...
export async function syncListingLogos(
  ...
  for (const entry of manifestEntries) {
    ...
    const extension = resolveLogoExtension(entry.sourceUrl, response.headers.get('content-type'))
    ...
    if (!currentProduct) {
      throw new Error(`Logo manifest entry "${entry.slug}" does not exist in ${sourcePath}.`)
    }
