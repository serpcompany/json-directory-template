...
function defaultConvertImpl(inputPath: string, outputPath: string): void {
  ...
  const magickInputPath =
    sourceExtension === '.gif' || sourceExtension === '.ico'
      ? `${inputPath}[0]`
      : inputPath;
  ...
  execFileSync(
    'magick',
    [magickInputPath, '-background', 'none', '-alpha', 'on', outputPath],
    {
      stdio: 'pipe',
    }
  );
...
export async function normalizeListingLogosToPng(
  ...
  for (const currentProduct of Object.values(products)) {
    ...
    const currentLogo =
      typeof currentMedia?.logo === 'string' ? currentMedia.logo : '';
...
async function main(): Promise<void> {
  const result = await normalizeListingLogosToPng(
    parseSiteInputArgs(process.argv.slice(2))
  );

  console.log(
    `Normalized ${result.convertedCount} listing logos to PNG for ${result.siteId}`
  );
