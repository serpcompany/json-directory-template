...
const DEFAULT_BADGE_DIR = join(
  REPO_ROOT,
  'apps',
  defaultSiteConfig.build.appPackageName,
  'public',
  'badge'
);
const SERPDOWNLOADERS_BADGE_DIR = join(
  REPO_ROOT,
  'apps',
  resolveCheckedInSiteConfig('serpdownloaders.com').build.appPackageName,
  'public',
  'badge'
);
...
<body>
  ...
  const expectedBadges = [
    { dir: DEFAULT_BADGE_DIR, filename: 'featured-on-default-light.svg' },
    { dir: DEFAULT_BADGE_DIR, filename: 'featured-on-default-dark.svg' },
    {
      dir: SERPDOWNLOADERS_BADGE_DIR,
      filename: 'featured-on-serpdownloaders.com-light.svg',
    },
    {
      dir: SERPDOWNLOADERS_BADGE_DIR,
      filename: 'featured-on-serpdownloaders.com-dark.svg',
    },
  ];

  for (const { dir, filename } of expectedBadges) {
    const badgePath = join(dir, filename);
    assert(
